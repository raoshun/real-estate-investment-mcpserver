...
def calculate_property_analysis(
    ...
    net_yield = calculate_net_yield(
        annual_rent, annual_expenses, purchase_price
    )
    
    # ローン関連計算
    loan_amount = property_data.get(
        'loan_amount', purchase_price * DEFAULT_LOAN_RATIO
    )
    ...
    # 投資回収期間
    down_payment = property_data.get(
    ...
    annual_depreciation = calculate_building_depreciation(
...
def validate_calculation_inputs(
    ...
    if 'loan_period' in property_data:
        ...
        if loan_period <= 0 or loan_period > 35:
            errors['loan_period'] = (
                "Loan period should be between 1 and 35 years"
            )
    ...
    if 'occupancy_months_per_year' in property_data:
        ...
        if occupancy < 0 or occupancy > 12:
            errors['occupancy_months_per_year'] = (
