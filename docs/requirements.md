...
## 現状実装との差異サマリー (Reality vs Plan)

| 項目 | 企画書記載 | 現状実装 | 備考 |
|------|-------------|----------|------|
| 物件収益性分析 | 表面/実質利回り, CF, DCF, ROI/IRR | 表面/実質利回り, CF, 回収期間, 減価償却, 税効果 | DCF/IRR 未実装 |
| リスク評価 | 立地/人口/競合/地価 | 未実装 | 将来外部データ連携予定 |
| ポートフォリオ | 分散/相関/リスク/税務最適化 | 総額/総月収/年間CF/目標達成度簡易 | 高度分析未実装 |
| 市場動向 | マクロ指標/スクリーニング | 未実装 | API 選定前 |
| ツール | property_valuation 他 4 種 | analyze_property / register_property / compare_properties / portfolio_analysis | 名前・範囲差異あり |
| 永続化 | データ層 | メモリ保持のみ | DB 未導入 |
| 設定 | settings.yaml 参照 | 雛形のみ・未参照 | 利用導線追加予定 |
| 推奨ロジック | 外部化 | ハードコード（利回り閾値） | 外部設定化予定 |
| テスト | 包括的 | 単体 + サーバーフォーマット | カバレッジ ~80% |

このセクションはアップデートのたびに同期し、Backlog 優先順位づけの基準とします。
